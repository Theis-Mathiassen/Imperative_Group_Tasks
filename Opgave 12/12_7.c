...
int tree_height_helper(tree_node *tree_ptr, int level);
...
int main(void) {
  ...
  printf("Tree height: %d", tree_height(tree4));
...
/* Opgave */
int tree_height(tree_node *tree_ptr){
    return tree_height_helper(tree_ptr, 0);
}
...
int tree_height_helper(tree_node *tree_ptr, int level){
    ...
    if (tree_ptr->leftp == NULL && tree_ptr->rightp == NULL) {
        ...
        result = fmax(tree_height_helper(tree_ptr->leftp, level + 1), tree_height_helper(tree_ptr->rightp, level + 1));
    } else if (tree_ptr->leftp != NULL) {
        result = tree_height_helper(tree_ptr->leftp, level + 1);
    } else {
        result = tree_height_helper(tree_ptr->rightp, level + 1);
    }
  return result;
}